...
export default function Post({ post, posts, reviewAuthorDetails, preview }) {
  ...
  const blogwriter = {
    ...
    ImageUrl: post?.content?.match(/<img[^>]*src='([^']*)'[^>]*\/>/)?.[1] || "/blog/images/author.png",
    description:
...
export const getStaticProps: GetStaticProps = async ({ params, preview = false, previewData }) => {
  ...
  const { techMoreStories } = await getMoreStoriesForSlugs(postData?.post?.tags, postData?.post?.slug);
  ...
  return {
    ...
    revalidate: 300, // Increased to 5 minutes
  };
};

export const getStaticPaths: GetStaticPaths = async () => {
  const allPosts = await getAllPostsForTechnology(false);
  const technologyPosts = allPosts?.edges?.map(({ node }) => `/technology/${node?.slug}`) || [];
  return {
    paths: technologyPosts.slice(0, 50), 
    fallback: "blocking",
  };
};