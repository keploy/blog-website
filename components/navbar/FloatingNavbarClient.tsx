            ...
            <div className="relative z-10 flex flex-col flex-1 overflow-hidden min-h-0" style={{ maxHeight: 'calc(100vh - 18rem)' }}>
              <div className="overflow-y-auto overscroll-contain px-5 py-5 flex-1 min-h-0">
                <div className="space-y-2.5">
                      ...
                      <div className="space-y-2 border-l-2 border-neutral-200/40 pl-4 ml-2">
                        {(techState.length
                          ...
                          if (!edge) {
                            return (
                              <div
                                key={`tech-mobile-skel-${index}`}
                                className="flex items-center w-full px-4 py-3 rounded-xl bg-white/40 ring-1 ring-neutral-200/40 animate-pulse min-h-[48px]"
                              >
                          ...
                          return (
                            ...
                            >
                              <span className="font-medium text-sm text-black/75 line-clamp-2">
                                {node.title}
                              </span>
                  ...
                  <Collapsible
                    open={mobileCommunityOpen}
                    onOpenChange={(open) => {
                      setMobileCommunityOpen(open);
                      if (open) {
                    ...
                    <CollapsibleContent className="mt-2.5 space-y-2">
                      <div className="space-y-2 border-l-2 border-neutral-200/40 pl-4 ml-2">
                        {(communityState.length
                          ? communityState.slice(0, 4)
                          : new Array(4).fill(null)
                        ).map((edge, index) => {
                          if (!edge) {
                            return (
                              <div
                                key={`community-mobile-skel-${index}`}
                                className="flex items-center w-full px-4 py-3 rounded-xl bg-white/40 ring-1 ring-neutral-200/40 animate-pulse min-h-[48px]"
                              >
                                <div className="h-3 w-3/4 bg-white/60 rounded" />
                              </div>
