              ...
              <div className="overflow-y-auto overscroll-contain px-5 py-5 flex-1 min-h-0">
                <div className="space-y-2.5">
                  ...
                  <Collapsible open={mobileTechOpen} onOpenChange={setMobileTechOpen}>
                    ...
                    <CollapsibleContent className="mt-2.5 space-y-2">
                      <div className="space-y-2 border-l-2 border-neutral-200/40 pl-4 ml-2">
                        {techState.slice(0, 4).map((edge, i) => {
                          if (!edge) return null;
                          const { node } = edge;
                          return (
                            <Link
                            ...
                            >
                              <div className="flex flex-col items-start text-left">
                                <span className="font-medium text-sm text-black/75 line-clamp-1">{node.title}</span>
                                <span className="text-[10px] text-neutral-500">{new Date(node.date).toLocaleDateString()}</span>
                              </div>
                        ...
                        <Link
                          href="/technology"
                          onClick={() => setMobileMenuOpen(false)}
                          className="block text-center text-xs font-semibold text-orange-600 py-2 hover:underline"
                        >
                          View All Technology →
                        </Link>
                      </div>
                  ...
                  <Collapsible open={mobileCommunityOpen} onOpenChange={setMobileCommunityOpen}>
                    ...
                    <CollapsibleContent className="mt-2.5 space-y-2">
                      <div className="space-y-2 border-l-2 border-neutral-200/40 pl-4 ml-2">
                        {communityState.slice(0, 4).map((edge, i) => {
                          if (!edge) return null;
                          const { node } = edge;
                          return (
                            <Link
