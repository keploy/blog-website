...
interface PostHeaderAuthorsProps {
   blogwriter: Author[];
   blogreviewer: Author[];
   timetoRead: number;
}

const PostHeaderAuthors = ({ blogwriter, blogreviewer, timetoRead }: PostHeaderAuthorsProps) => {
   const sameAuthor = blogwriter[0].name.split(" ")[0].toLowerCase() === blogreviewer[0].name.toLowerCase();
      ...
      <>
         <div className="flex flex-col lg:flex-row lg:mt-7 items-start sm:items-center sm:justify-around gap-4 sm:gap-0  sm:px-0 lg:mx-28">
            ...
            <div className="flex flex-row gap-5 items-center gap-3 sm:gap-5 order-2 sm:order-none mt-2 md:mb-2">
               ...
               {copied && <span className="ml-2 text-orange-500 text-sm">Copied!</span>}
            </div>
         </div>
