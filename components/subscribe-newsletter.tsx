...
export const subscribeMutation = (formData: {
   fullName: string;
   email: string;
   companyName: string;
   message: string;
   ...
   useEffect(() => {
      ...
      const timer = setTimeout(() => {
         if (!props.isSmallScreen) {
            gsap.to(currentBunnyRef, { y: -180, duration: 1, yoyo: true, repeat: 1 }).then(() => {
   ...
   return (
      <div className="flex flex-col" ref={bunnyRef}>
         ...
         <div className="hidden lg:block ">
            <Image src={newsletterBunny} alt="Keploy mascot - newsletter subscription illustration" />
         </div>
         <div
         ...
         >
            <div className={`${isVisible ? styles["slide-in"] : "translate-x-full opacity-0"}`}>
               ...
               {subscribed && (
                  <p className="text-sm text-green-800 text-center font-semibold mt-3">Thanks for subscribing!</p>
               )}
            </div>
         </div>
      </div>
