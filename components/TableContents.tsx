...
function TOCItem({ id, title, type, onClick }) {
  ...
  return (
    <li className={itemClasses} style={{ marginLeft }}>
      <button
        onClick={() => onClick(id)}
        className="block py-1 w-full rounded-md text-left text-sm text-black hover:text-orange-500 opacity-75 hover:opacity-100 transition-all duration-150 ease-in-out"
      >
        {title}
      </button>
    </li>
...
export default function TOC({ headings }) {
  ...
  return (
    <>
      <div className="inline-block lg:hidden left-0 top-20 p-4">
      ...
      <div className="hidden lg:inline-block left-0 top-20 overflow-y-auto h-auto bg-inherit p-4 sticky max-h-[90vh]">
        <div className="text-lg font-semibold mb-2">Table of Contents</div>
        <nav>
