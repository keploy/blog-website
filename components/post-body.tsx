...
export default function PostBody({
   ...
   const sameAuthor =
      authorName.split(" ")[0].toLowerCase() === ReviewAuthorDetails.edges[0].node.name.split(" ")[0].toLowerCase();
   ...
   useEffect(() => {
      ...
      let initialReplacedContent = content.replace(/<div class="post-toc-header">[\s\S]*?<\/div>/gm, "");
   ...
   const handleHeadingCopyClick = useCallback(
   ...
   const renderCodeBlocks = () => {
      ...
      return replacedContent.split(/(<pre[\s\S]*?<\/pre>)/gm).map((part, index) => {
   ...
   return (
      <div className={`flex flex-col ${isList ? "items-center" : "items-center lg:items-start lg:flex-row"} `}>
         ...
         <div className={`w-full p-4 ${isList ? "ml-10" : ""}  md:w-4/5 lg:w-3/5`} id="post-body-check">
            ...
            <div className="my-5">
               <AuthorDescription authorData={content} AuthorName={authorName} isPost={true} />
            </div>
            {!sameAuthor && (
               <div className="my-20">
